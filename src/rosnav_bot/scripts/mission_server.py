...
Send a mission from the command line (daemon must be running):
  ros2 run rosnav_bot mission_server.py patrol  robot1 room_a room_b room_c
  ros2 run rosnav_bot mission_server.py patrol  robot1 1,2,0  3,4,90
  ros2 run rosnav_bot mission_server.py sequence robot1 0,0,0 2,0,0 2,2,90
  ros2 run rosnav_bot mission_server.py goto    robot1 room_a
  ...
  ros2 run rosnav_bot mission_server.py locations
...
def _locations_path() -> str:
    try:
        ...
        return os.path.join(
            get_package_share_directory('rosnav_bot'), 'config', 'locations.yaml')
    except Exception:
        return os.path.join(
            os.path.expanduser('~'), 'rosnav', 'src',
            'rosnav_bot', 'config', 'locations.yaml')
